...
export default async function BlogsPage({ searchParams }: BlogsPageProps) {
  ...
  return (
    <main className="min-h-screen bg-[var(--color-background)] text-[var(--color-foreground)]">
      <section className="border-b border-[var(--color-border-subtle)] bg-[radial-gradient(circle_at_top_left,_rgba(201,168,76,0.16),_transparent_45%),linear-gradient(135deg,_var(--color-surface-elevated),_var(--color-background))]">
        <div className="max-w-7xl mx-auto px-6 py-16 lg:py-24">
          <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-10 items-end">
            ...
            <div className="rounded-sm border border-[var(--color-border-subtle)] bg-[var(--color-surface-elevated)]/90 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.45)]">
              <p className="text-xs  tracking-[0.25em] text-gold mb-3">Fresh from the studio</p>
              <p className="text-xl font-serif text-[var(--color-foreground)]">Thoughtful narratives, timeless craftsmanship, and cultural depth.</p>
            </div>
          </div>
        </div>
      </section>
